...
namespace CsvDataExtraction{
	...
	public:
		...
		/** Customized destructor to delete all available vectors and release the memory*/
		...
		/** Getters and setters for the instance variables recognized in the DTO class*/
		...
		__declspec(dllexport) vector<int> Order() const { return order; }
		__declspec(dllexport) void Order(vector<int> val) { order = val; }
		...
		/** Initialize the vectors to null for the destructing purposes at the destructor*/
		void Init(){
