...
void CIntelliCheckersUIDlg::Descritize()
{
	//NullPreProcessor eliminator(m_source);
	//eliminator.elimiateNullValues();
	//m_source = eliminator.NullEliminatedDatasource();
}
void CIntelliCheckersUIDlg::RangeSplite()
{
	//NullPreProcessor eliminator(m_source);
	//eliminator.elimiateNullValues();
	//m_source = eliminator.NullEliminatedDatasource();
...
void CIntelliCheckersUIDlg::OnFlexButtonClick(CFlexEvent *evt, CString controller)
{
	...
	{	
		...
		else if(data_to_filter_to_apriory_to_textview_procedures)
		{
			genericProcedure=DATA_FILTER_APRIORY_TEXT
		}
		else if(data_to_filter_to_classification_to_textview_procedures)
		{
			genericProcedure=DATA_FILTER_CLASSIFICATION_TEXT
		}
		else if(data_to_filter_to_classification_to_treeview_procedures)
		{
			genericProcedure=DATA_FILTER_CLASSIFICATION_TREE
		}
		else if(data_to_filter_to_compression_to_apriory_to_textview_procedures)
		{
			genericProcedure=DATA_FILTER_COMPRESSION_APRIORY_TEXT
		}
		else if(data_to_filter_to_compression_to_classification_to_textview_procedures)
		{
			genericProcedure=DATA_FILTER_COMPRESSION_CLASSIFICATION_TEXT
		}
		else if(data_to_filter_to_compression_to_classification_to_treeview_procedures)
		{
		...
		if(validProcedure)
		{
			...
			for(int i=0;i<loopCount;i++)
			{
				...
				if(runInALoop=="false")
				{
					...
					else if(genericProcedure==DATA_FILTER_COMPRESSION_APRIORY_TEXT || data_to_apriory_to_textview_procedures || data_to_compression_to_apriory_to_textview_procedures || data_to_filter_to_apriory_to_textview_procedures)//
					{
						formattedOutPut += Text(APRIORI_SOURCE,0);
					}
					else if(genericProcedure==DATA_FILTER_COMPRESSION_CLASSIFICATION_TEXT || data_to_classification_to_textview_procedures || data_to_compression_to_classification_to_textview_procedures || data_to_filter_to_classification_to_textview_procedures)//
					{
						formattedOutPut += Text(CLASSIFIER_TEXT_SOURCE,0);
					}
					else if(genericProcedure==DATA_FILTER_COMPRESSION_CLASSIFICATION_TREE || data_to_classification_to_treeview_procedures || data_to_compression_to_classification_to_treeview_procedures || data_to_filter_to_classification_to_treeview_procedures)//
					{
